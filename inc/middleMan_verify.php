              ...
              if(strlen($password) < 9) {
                ...
                if ($verifyquery) {
                  ...
                  try {
                    ...
                    //Recipients
                    $mail->setFrom(EMAIL, 'JOENEST'); //web app name
                    $mail->addAddress($mailTo);                   
                    $mail->addReplyTo('no-reply@gmail.com', 'No Reply');   

                    // Content
                    $url = "http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/inc/verify.php?token=$token";

                    $mail->isHTML(true);                                  
                    $mail->Subject = 'E-mail verification from JOENEST';
                    $mail->Body    = "<h4 align=center>
                                        Feather Text wants you to verify your Email
                                        Address concerning the registration. Kindly verify
                                        your Email Address through the link below
                                        <br>
                                        Link : click <a href='$url'>verify</a> to do so
                                      </h4>";
                    $mail->send();
                    echo "<script>alert('Thank you for Registering with Feather Text. We have sent a verification E-mail to the address provided')</script>";
      ...
      if(mysqli_num_rows($query) > 0) {
        ...
        if(password_verify($pass, $password_hash)) {
          ...
          if ($user_auth == 1) {
            ...
            echo "<script>alert('Error. This account has not been verified.')</script>";
            echo "<script>window.open('index.php', '_self')</script>";
            exit();
          }
        }
